    ...
    for (const stat of stats) {
        switch (stat.statType) {
            ...
            case StatType.EXALTED_WIS:
                ...
                continue;
            case StatType.POTION_ONE_TYPE:
                playerData.potionOneType = stat.statValue;
                continue;
            case StatType.POTION_TWO_TYPE:
                playerData.potionOneType = stat.statValue;
                continue;
            case StatType.POTION_THREE_TYPE:
                playerData.potionOneType = stat.statValue;
                continue;
            case StatType.POTION_BELT:
                playerData.potionBelt = (stat.statValue == 1);
                continue;
            case StatType.FORGEFIRE:
            ...
            default:
                if (stat.statType >= StatType.INVENTORY_0_STAT && stat.statType <= StatType.INVENTORY_11_STAT) {
                    ...
                    //Logger.log('StatData', `Unhandled StatType ${stat.statType} received, value: ${stat.statValue} (extra byte: ${stat.magicByte})`);
                }
        }
    }
    return playerData;
